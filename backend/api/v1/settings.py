...
async def test_api_key(request: ApiKeyTestRequest) -> ApiKeyTestResponse:
    ...
    try:
        ...
        success = llm_manager.test_provider_connection(provider_type, request.api_key, request.model_name, **provider_kwargs)
...
@router.post("/refresh-openai-models")
async def refresh_openai_models(request: RefreshModelsRequest) -> RefreshModelsResponse:
    """从OpenAI兼容端点（如ProxyPal）动态刷新模型列表"""
    try:
        ...
        return RefreshModelsResponse(success=False, error=str(e), models=[])
