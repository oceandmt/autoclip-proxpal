...
class OpenAIProvider(LLMProvider):
    """OpenAI提供商（支持自定义OpenAI兼容base_url，如ProxyPal）"""
    
    def __init__(self, api_key: str, model_name: str = "gpt-3.5-turbo", **kwargs):
        super().__init__(api_key, model_name, **kwargs)
        try:
            import openai
            base_url = kwargs.get("base_url") or os.getenv("OPENAI_BASE_URL") or os.getenv("PROXYPAL_BASE_URL")
    ...
    def _fetch_remote_models(self) -> List[ModelInfo]:
        ...
        try:
            ...
            remote_models: List[ModelInfo] = []
            for item in items:
                model_id = item.get("id")
                if not model_id:
                    continue
                owned_by = item.get("owned_by") or "ProxyPal"
                remote_models.append(
                    ModelInfo(
                        ...
                        max_tokens=128000,
                        description=f"Dynamic model from {owned_by} via OpenAI-compatible endpoint"
                    )
                )
            return remote_models
        except Exception as e:
            logger.warning(f"动态获取OpenAI兼容模型列表失败{self._extra_log_context()}: {e}")
            return []
    ...
    def call(self, prompt: str, input_data: Any = None, **kwargs) -> LLMResponse:
        ...
        try:
            ...
            logger.error(f"OpenAI调用失败{self._extra_log_context()}: {str(e)}")
            raise
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            logger.error(f"OpenAI连接测试失败{self._extra_log_context()}: {e}")
            return False
    
    def get_available_models(self) -> List[ModelInfo]:
        """获取OpenAI可用模型；如配置了OpenAI兼容base_url则优先动态拉取"""
