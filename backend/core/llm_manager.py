...
class LLMManager:
    ...
    def _get_provider_kwargs(self, provider_type: ProviderType) -> Dict[str, Any]:
        ...
        if provider_type == ProviderType.OPENAI:
            base_url = self.settings.get("openai_base_url") or os.getenv("OPENAI_BASE_URL") or os.getenv("PROXYPAL_BASE_URL")
    ...
    def test_provider_connection(self, provider_type: ProviderType, api_key: str, model_name: str, **provider_kwargs) -> bool:
        """测试提供商连接"""
        try:
            effective_kwargs = self._get_provider_kwargs(provider_type)
            effective_kwargs.update({k: v for k, v in provider_kwargs.items() if v})
            provider = LLMProviderFactory.create_provider(provider_type, api_key, model_name, **effective_kwargs)
    ...
    def get_all_available_models(self) -> Dict[str, List[Dict[str, Any]]]:
        """获取所有可用模型；当前提供商如已配置则优先使用实时模型列表"""
        all_models = LLMProviderFactory.get_all_available_models()
        result = {}

        current_provider_name = self.settings.get("llm_provider")
        current_provider_type = None
        if current_provider_name:
            try:
                current_provider_type = ProviderType(current_provider_name)
            except ValueError:
                current_provider_type = None
        ...
        for provider_type, models in all_models.items():
            ...
            if current_provider_type == provider_type and self.current_provider:
                try:
                    dynamic_models = self.current_provider.get_available_models()
                    if dynamic_models:
                        effective_models = dynamic_models
                except Exception as e:
                    logger.warning(f"获取{provider_name}动态模型列表失败: {e}")
