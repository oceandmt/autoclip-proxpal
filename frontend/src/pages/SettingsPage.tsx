...
const SettingsPage: React.FC = () => {
  ...
  const providerConfig = {
    ...
    openai: {
      name: 'OpenAI / ProxyPal',
      icon: <RobotOutlined />,
      color: '#52c41a',
      description: 'OpenAI兼容接口，可直连OpenAI或ProxyPal',
      apiKeyField: 'openai_api_key',
      baseUrlField: 'openai_base_url',
      placeholder: '请输入OpenAI或ProxyPal API密钥'
    },
    gemini: {
      name: 'Google Gemini',
      icon: <RobotOutlined />,
  ...
  const refreshProxyPalModels = async (silent: boolean = false) => {
    const apiKey = form.getFieldValue('openai_api_key')
    ...
    try {
      const result = await settingsApi.refreshOpenAIModels(apiKey, baseUrl)
      if (result.success) {
        setAvailableModels((prev: any) => ({ ...prev, openai: result.models }))
        if (!silent) {
          message.success(`已刷新 ${result.models.length} 个 ProxyPal 模型`)
        }
      } else if (!silent) {
        message.error('刷新模型列表失败: ' + (result.error || '未知错误'))
      }
    } catch (error: any) {
      if (!silent) {
        message.error('刷新模型列表失败: ' + (error.message || '未知错误'))
      }
    }
  }
  ...
  const handleTestApiKey = async () => {
    const selectedConfig = providerConfig[selectedProvider as keyof typeof providerConfig] as any
    const apiKey = form.getFieldValue(selectedConfig.apiKeyField)
    const modelName = form.getFieldValue('model_name')
    const baseUrl = selectedConfig.baseUrlField ? form.getFieldValue(selectedConfig.baseUrlField) : undefined
  ...
  const handleProviderChange = async (provider: string) => {
    ...
    if (provider === 'openai') {
      ...
      const openaiBaseUrl = form.getFieldValue('openai_base_url')
      if (openaiApiKey && openaiBaseUrl) {
        await refreshProxyPalModels(true)
      }
    }
  }
            ...
            <Card title="AI 模型配置" className="settings-card">
              ...
              >
                ...
                {selectedProvider === 'openai' && (
                  <>
                    <Form.Item
                      label="OpenAI / ProxyPal Base URL（可选）"
                      name="openai_base_url"
                      className="form-item"
                      extra="直连OpenAI时可留空；使用ProxyPal时填写 http://127.0.0.1:8317/v1"
                    >
                      <Input
                        placeholder="例如: http://127.0.0.1:8317/v1"
                        prefix={<ApiOutlined />}
                        className="settings-input"
                        onChange={() => {
                          if (refreshTimerRef.current) {
                    ...
                    <Form.Item className="form-item">
                      <Button
                        onClick={() => refreshProxyPalModels(false)}
                        icon={<ApiOutlined />}
                        disabled={!form.getFieldValue('openai_api_key') || !form.getFieldValue('openai_base_url')}
                      >
                        从 ProxyPal 刷新模型列表
                      </Button>
                    </Form.Item>
