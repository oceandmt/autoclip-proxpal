...
export const settingsApi = {
  ...
  // 从OpenAI兼容端点（如ProxyPal）刷新模型列表
  refreshOpenAIModels: (apiKey: string, baseUrl: string): Promise<{ success: boolean; models: any[]; error?: string }> => {
    return api.post('/settings/refresh-openai-models', {
      api_key: apiKey,
      base_url: baseUrl,
    })
  },
